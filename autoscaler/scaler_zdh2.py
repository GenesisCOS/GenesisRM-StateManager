...
class MicroKubeScaler(Scaler):
    ...
    def start_l1_scaling(self, service_name: str, endpoint_name: str, results, results_lock):            
        predict_value, upper_bound, _, last_value, use_time = self.sttf_estimator.predict(service_name, endpoint_name, self.start_time)
    ...
    def start(self):
        ...
        for svc in services:
            if svc.get('name') in exclude_services:
                continue
            if svc.get('name') not in self.service_configs:
                continue
